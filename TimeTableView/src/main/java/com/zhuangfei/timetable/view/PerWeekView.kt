 ...
 */
class PerWeekView @JvmOverloads constructor(context: Context?, attrs: AttributeSet? = null) : View(context, attrs) {
    //控件宽度
    private var width = 0
    ...
    fun setData(list: List<Schedule>?, curWeek: Int): PerWeekView {
        ...
        for (i in list.indices) {
            val schedule = list[i]
            if (ScheduleSupport.isThisWeek(schedule, curWeek) && schedule.start <= 10 && schedule.day <= 5) {
